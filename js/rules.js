...
function isPathBlocked(state,from,to){var n=state.columns.length,a=posCoord(from,n),b=posCoord(to,n);if(a===b)return false;var lo=Math.min(a,b),hi=Math.max(a,b);for(var i=0;i<n;i++){if(i>lo&&i<hi&&state.columns[i].length===4)return true;}return false;}
function canMove(state,from,to){if(from===to)return {ok:false,reason:'same'};var token=sourceToken(state,from);if(!token)return {ok:false,reason:'empty'};if(!destHasSpace(state,to))return {ok:false,reason:'full'};if(isPathBlocked(state,from,to))return {ok:false,reason:'blocked'};return {ok:true,token:token};}
...
window.SunnyRules={cloneState,sourceToken,destHasSpace,isPathBlocked,canMove,movePiece,getLegalMoves,isSolved,isDeadlocked,serializeState,hashState,countMixedColumns,heuristic};
})();
